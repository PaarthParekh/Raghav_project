    ...
    def __data_generation(self):
        ...
        for i in range(N_events):
            ...
            if N_codes >= N_sub: 
                ...
                sta_names = stations[["code"]][stations["code"].isin(station_codes)].values[selection]
                waveforms[i] = event_data[selection]
                station_coords[i, :, 0, :2] = coords
                station_selection.append(sta_names.T[0])
            else:
                waveforms[i, :N_codes] = event_data
                coords = stations[["lat", "lon"]][stations["code"].isin(station_codes)].values
                sta_names = stations[["code"]][stations["code"].isin(station_codes)].values
        ...
        self.selected_stations = station_selection
        pass
